...
class RateLimiter:
    ...
    async def check(self, client_id):
        ...
        # Remove old requests
        client_requests = [req for req in client_requests if now - req < self.per_seconds]
...
async def rate_limit_middleware(request: Request, call_next):
    ...
    if request.url.path == "/health" or client_ip.startswith("10.") or client_ip.startswith("172."):
...
async def get_job_status(
    job_id: str,
    details: bool = Query(False, description="Show detailed file statuses")
):
...
async def list_jobs_page(page_number: int = 0, status: str = "all"):
    ...
    jobs_list = await run_in_threadpool(tracker.list_jobs, page_number=page_number, status=status)
    return {
            "total": jobs_count,
            "jobs": jobs_list,
    }
