...
class JobTracker:
            ...
            try:
                ...
                if not details:
                    return {
                        ...
                        "progress": data["summary"]["progress"] if "summary" in data else 0.0,
                        "created_at": data["created_at"] if "created_at" in data else "unknown",
                        "updated_at": data["updated_at"] if "updated_at" in data else "unknown",
    ...
    def list_jobs(self, page_number=0, status="all", sort_by="created_at", sort_order="desc"):
        ...
        with self.global_lock:
            ...
            for job_file in job_files:
                ...
                try:
                    ...
                    if status == "all" or status == data["status"]:
                        jobs_meta.append({
                            "job_id": data["job_id"] if "job_id" in data else job_file_id,
            ...
            sorted_jobs = sorted(jobs_meta, key=lambda x: x.get(sort_by, ""), reverse=reverse)
    ...
    def cancel_job(self, job_id):
        ...
        with job_lock:
            ...
            return self.update_job(job_id, lambda data: data.update({
                **data,
                "status": "cancelled",
        ...
        for job_id in job_ids:
            ...
            if job_data.get('status') == 'processing':
                ...
                if (now - updated_at).total_seconds() > 1800:  # 30 minutes
                    ...
                    with job_lock:
                        if self.update_job(job_id, lambda data: data.update({
                            "status": "pending",
