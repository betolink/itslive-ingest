    ...
    try:
        ...
        for filename in files:
            ...
            proc = await asyncio.create_subprocess_exec(
                'pypgstac', 'load', 'collections', file_path,
                f'--dsn={DATABASE_URL}', '--method=insert_ignore',
...
async def load_queryables(index_fields: list):
    ...
    try:
        ...
        for filename in files:
            ...
            proc = await asyncio.create_subprocess_exec(
                'pypgstac', 'load_queryables', file_path,
                f'--dsn={DATABASE_URL}',
            ...
            stdout, stderr = await proc.communicate()
            if proc.returncode != 0:
                raise Exception(f"Database migration for queryables failed: {stderr.decode().strip()}")
...
async def initialize_database_task(job_id: str,
                                   migrate: bool):
    try:
        tracker.update_job(job_id, lambda data: data.update({
            **data,
            "status": "processing",
        ...
        if migrate:
            ...
            tracker.update_job(job_id, lambda data: data.update({
                **data,
                "message": "Built-in database migrations complete"
            }))
...
async def process_files(job_id: str):
    try:
        ...
        tracker.update_job(job_id, lambda data: data['summary'].update({
            **data['summary'],
            "total_files": len(files),
            "progress": 0.0
        }))

        for batch_start in range(0, len(files), MAX_CONCURRENT_FILES):
            ...
            batch_end = min(batch_start + MAX_CONCURRENT_FILES, len(files))
            batch_tasks = [
                process_file(job_id, bucket, key, index, len(files)) for index in range(batch_start, batch_end)
            ]
...
async def process_file(job_id: str, bucket: str, key: str, index: int, total: int):
    ...
    try:
        ...
        if existing:
            tracker.update_job(job_id, lambda data: {
                "summary": {
        ...
        tracker.update_job(job_id, lambda data: data['details'].update({
            s3_path: {
        ...
        async with PROCESS_SEM:
            ...
            try:
                ...
                raise Exception(f"Invalid count output: {stdout.decode()}")

            proc = await asyncio.create_subprocess_exec(
                'pypgstac', 'load', 'items', str(tmp_path),
...
def check_existing_ingest(job_id, bucket, key, size_mb, etag):
    ...
    for past_job_id in tracker.list_jobs():
        ...
        if file_entry and file_entry['status'] == 'success':
            if (file_entry['size_mb'] == size_mb and 
                file_entry['etag'] == etag):
                return True
    return False
